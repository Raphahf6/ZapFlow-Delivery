...
function CartDrawer({ isOpen, onClose, cart, company, onSuccess }: any) {
    ...
    const handleFinish = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // NOVA LÓGICA: DISPARO DE WHATSAPP (PEDIDO CONFIRMADO)
            try {
                const mensagemCliente = `*Pedido Confirmado!* 🎉\n\nOlá, ${customerName}!\nRecebemos seu pedido na *${company.name}* e ele já foi para a cozinha/preparo.\n\n*Resumo:*\n${cart.map((item: any) => `${item.quantity}x ${item.name}`).join('\n')}\n\n*Total:* R$ ${total.toFixed(2).replace('.', ',')}\n*Pagamento:* ${paymentMethod}\n\nAvisaremos por aqui assim que sair para entrega! 🛵`;
                ...
                console.error("Erro ao enviar WhatsApp de confirmação:", wppError);
                // Não damos throw aqui para não travar o fluxo caso o wpp falhe
            }
